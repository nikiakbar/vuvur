...
def get_media_row(media_id):
    ...
    try:
        conn = get_db()
        c = conn.cursor()
        # ⚡ Bolt: Selecting only required columns to avoid loading large EXIF blobs into memory.
...
def thumb(mid):
    ...
    # ⚡ Bolt: Fast-path for already generated thumbnails.
    # We attempt to serve the thumbnail directly from disk before hitting the database.
    # This reduces DB load and latency for the most common request path.
    for ext in (".jpg", ".gif"):
        try:
