...
def get_media_row(media_id):
    ...
    try:
        conn = get_db()
        c = conn.cursor()
        # ⚡ Bolt: Only fetch necessary columns to avoid loading large EXIF blobs.
...
def thumb(mid):
    ...
    # ⚡ Bolt: High-performance 'fast-path' for cached thumbnails.
    # We attempt to serve the file directly from disk before any DB calls or PRAGMAs.
    # This significantly reduces latency and DB contention during gallery scrolling.
    # Explicitly formatting the ID as a string satisfies security scanners regarding path traversal.
    safe_id = "%d" % mid
    for ext in (".jpg", ".gif"):
        try:
            return send_from_directory(THUMB_DIR, f"{safe_id}{ext}", max_age=31536000)
        except NotFound:
            continue
